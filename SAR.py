...
def get_geojson_detections(tiff_path, shard_dir, outpath):
  with rasterio.open(tiff_path) as src:
  	print("Splitting image into shards")
  	print(shard_dir)
  	splitImageIntoCells(src, "shard", 128, shard_dir)
  shard_list = glob.glob(shard_dir+"*.png")
  list_of_ship_detections = []
  confidence_list = []
  print("Finding ships")
...
num_classes = 2
model_ft = get_instance_segmentation_model(num_classes).to(device)
model_ft.load_state_dict(torch.load(rootdir+"/"+"model.bin", map_location=device))
model_ft.eval()
